...
const assets = {
  ...
  professional: `${aem}18ea2a62-8993-4715-a998-6360fcfc080c/as/CSO3050PE_S_P-1.avif?assetname=CSO3050PE_S_P+1.png&width=1000&max-quality=90`,
  comparison: `${aem}bc8cfbe9-3c5d-4c31-9424-4cafe815949b/as/CSO_Comparison_scaled.avif?assetname=CSO_Comparison_scaled.png&width=1000&max-quality=90`,
...
type SteamOven = {
  ...
  title: string;
  width: string;
  finishes: readonly ("Black" | "Stainless Steel")[];
  image: string;
  count?: string;
};

const models: readonly SteamOven[] = [
  { model: "CSO50CM", title: "M Series Contemporary Convection Steam Oven", width: '24"', finishes: ["Black", "Stainless Steel"], image: assets.comparison, count: "1 / 2" },
  { model: "CSO50PE", title: "E Series Professional Convection Steam Oven", width: '30"', finishes: ["Stainless Steel"], image: assets.professional },
  { model: "CSO50PM", title: "M Series Professional Convection Steam Oven", width: '30"', finishes: ["Stainless Steel"], image: assets.professional },
  { model: "CSO50TE", title: "E Series Transitional Convection Steam Oven", width: '24" 30"', finishes: ["Stainless Steel"], image: assets.comparison, count: "1 / 2" },
  { model: "CSO50TM", title: "M Series Transitional Convection Steam Oven", width: '24" 30"', finishes: ["Stainless Steel"], image: assets.comparison },
];

function SteamOvenCard({ oven, priority }: { oven: SteamOven; priority?: boolean }) {
  return (
    <Link href={`/products/${oven.model.toLowerCase()}`} className="group flex min-w-0 flex-col border border-[#d2cdc1] bg-[#f7f5ef] focus:outline-none focus-visible:ring-1 focus-visible:ring-[#181817]">
      <div className="relative aspect-[1/1.18] overflow-hidden bg-[#f4f2ec]">
        <Image src={oven.image} alt={oven.title} fill priority={priority} sizes="(min-width: 1024px) 31vw, (min-width: 640px) 48vw, 92vw" className="object-contain px-8 py-10 transition duration-500 group-hover:scale-[1.025] md:px-12 md:py-14" />
      ...
      <div className="flex flex-1 flex-col px-5 pb-6 pt-5 md:px-6">
        ...
        <dl className="mt-5 border-t border-[#d2cdc1] text-[11px] leading-[1.35]">
          ...
          <div className="grid min-h-[52px] grid-cols-[42%_1fr] border-b border-[#d2cdc1] py-2.5"><dt>Finish:</dt><dd className="space-y-1">{oven.finishes.map((finish) => <span key={finish} className="flex items-center gap-2"><span className={`size-3 rounded-full border border-black/15 ${finish === "Black" ? "bg-[#242424]" : "bg-[#aaa9a5]"}`} aria-hidden="true" />{finish}</span>)}</dd></div>
          <div className="grid grid-cols-[42%_1fr] py-2.5"><dt>Features:</dt><dd>Plumbed optional</dd></div>
        </dl>
      </div>
    </Link>
  );
...
export function ConvectionSteamOvensPage() {
  return (
    <main className="bg-[#f4f2ec] text-[#181817]">
      ...
      <section className="px-6 pb-20 md:px-12 md:pb-24"><div className="mx-auto max-w-[1392px]">
        <div className="flex flex-wrap items-center justify-between gap-5 border-b border-[#c9c4ba] pb-3"><div className="flex flex-wrap gap-x-12 gap-y-2 text-[13px]"><span className="inline-flex min-h-9 items-center gap-4">Series (1) <span aria-hidden="true">⌄</span></span><span className="inline-flex min-h-9 items-center gap-4">Width <span aria-hidden="true">⌄</span></span><span className="inline-flex min-h-9 items-center gap-4">Finish <span aria-hidden="true">⌄</span></span></div><div className="flex items-center gap-7 text-[12px] font-light"><span>All filters (1)</span><span className="h-7 w-px bg-[#c9c4ba]" /><span>Sort by ↓</span></div></div>
        <div className="mt-4 border-b border-[#c9c4ba] pb-5"><p className="text-[9px] font-semibold uppercase tracking-[0.22em]">Selected filters (1)</p><div className="mt-3 flex flex-wrap items-center gap-2 text-[12px]"><span className="rounded-full border border-[#8d877d] px-4 py-2 font-semibold">Convection Steam Oven ×</span><button type="button" className="ml-2 underline underline-offset-4">Clear all</button></div></div>
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3">{models.map((oven, index) => <SteamOvenCard key={oven.model} oven={oven} priority={index < 3} />)}</div><p className="mt-10 text-center text-[11px] font-semibold">Viewing 5 of 5</p>
      </div></section>
