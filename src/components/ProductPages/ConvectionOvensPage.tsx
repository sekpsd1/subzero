...
const assets = {
  ...
  doubleOven: `${aem}6555c214-3930-4677-bc7c-081cd9f8f8d1/as/DO_Comparison_scaled.avif?assetname=DO_Comparison_scaled.png&width=900&max-quality=90`,
  singleOven: `${aem}6c1ca93c-5e90-4295-8e6f-1ddb85acc61e/as/Wolf-Convection-Oven.avif?assetname=Wolf+Convection+Oven.jpg&width=900&max-quality=90`,
...
const cookingCategories = [
  ...
  ["Coffee systems", "/cooking/coffee-systems"], ["Microwaves", "/products/cooking/microwaves"],
...
const models: readonly OvenModel[] = [
  { title: "M Series Contemporary Built-In Double Oven", model: "DO50CM", configuration: "double", widths: '30"', finishes: ["Black", "Stainless Steel"], count: "1 / 2" },
  { title: "E Series Professional Built-In Double Oven", model: "DO50PE", configuration: "double", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "M Series Professional Built-In Double Oven", model: "DO50PM", configuration: "double", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "E Series Transitional Built-In Double Oven", model: "DO50TE", configuration: "double", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "M Series Transitional Built-In Double Oven", model: "DO50TM", configuration: "double", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "M Series Contemporary Built-In Single Oven", model: "SO50CM", configuration: "single", widths: '30"', finishes: ["Black", "Stainless Steel"], count: "1 / 2" },
  { title: "E Series Professional Built-In Single Oven", model: "SO50PE", configuration: "single", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "M Series Professional Built-In Single Oven", model: "SO50PM", configuration: "single", widths: '30"', finishes: ["Stainless Steel"] },
  { title: "E Series Transitional Built-In Single Oven", model: "SO50TE", configuration: "single", widths: '24" 30"', finishes: ["Stainless Steel"], count: "1 / 2" },
  { title: "M Series Transitional Built-In Single Oven", model: "SO50TM", configuration: "single", widths: '30"', finishes: ["Stainless Steel"] },
];

function FilterButton({ children }: { children: string }) {
  return <button type="button" className="inline-flex min-h-9 items-center gap-4 text-[13px] font-light">{children}<span aria-hidden="true">⌄</span></button>;
}

function OvenCard({ oven, priority }: { oven: OvenModel; priority?: boolean }) {
  return (
    <Link href={`/products/${oven.model.toLowerCase()}`} className="group flex min-w-0 flex-col border border-[#d2cdc1] bg-[#f7f5ef] focus:outline-none focus-visible:ring-1 focus-visible:ring-[#181817]">
      <div className="relative aspect-[1/1.18] overflow-hidden bg-[#f4f2ec]">
        <Image src={oven.configuration === "double" ? assets.doubleOven : assets.singleOven} alt={oven.title} fill priority={priority} sizes="(min-width: 1024px) 31vw, (min-width: 640px) 48vw, 92vw" className={`object-contain px-8 py-10 transition duration-500 md:px-12 md:py-14 ${oven.configuration === "double" ? "scale-[1.78] group-hover:scale-[1.81]" : "group-hover:scale-[1.02]"}`} />
      ...
      <div className="flex flex-1 flex-col px-5 pb-6 pt-5 md:px-6">
        <h2 className="min-h-[48px] max-w-[330px] font-serif text-[20px] font-light leading-[1.12] underline-offset-4 group-hover:underline">{oven.title}</h2>
        <dl className="mt-5 border-t border-[#d2cdc1] text-[11px] leading-[1.35]">
          <div className="grid grid-cols-[42%_1fr] border-b border-[#d2cdc1] py-2.5"><dt>Width:</dt><dd>{oven.widths}</dd></div>
          <div className="grid min-h-[52px] grid-cols-[42%_1fr] py-2.5"><dt>Finish:</dt><dd className="space-y-1">{oven.finishes.map((finish) => <span key={finish} className="flex items-center gap-2"><span className={`size-3 rounded-full border border-black/15 ${finish === "Black" ? "bg-[#242424]" : "bg-[#aaa9a5]"}`} aria-hidden="true" />{finish}</span>)}</dd></div>
        </dl>
      </div>
    </Link>
  );
...
function DiscoveryCard({ title, copy, cta, href, image }: { title: string; copy: string; cta: string; href: string; image: string }) {
  return <article className="bg-[#f8f6f1]"><div className="relative aspect-[1.62] overflow-hidden"><Image src={image} alt="" fill sizes="(min-width: 768px) 50vw, 100vw" className="object-cover" /></div><div className="grid gap-6 p-7 md:min-h-[205px] md:grid-cols-[0.48fr_0.52fr] md:p-8"><div className="flex flex-col items-start justify-between gap-7"><h3 className="font-serif text-[32px] font-light leading-[1.02] md:text-[38px]">{title}</h3><Link href={href} className="inline-flex min-h-10 items-center rounded-full border border-[#181817] px-6 text-[12px] font-semibold">{cta}</Link></div><p className="text-[14px] font-light leading-[1.35]">{copy}</p></div></article>;
}

export function ConvectionOvensPage() {
  return (
    <main className="bg-[#f4f2ec] text-[#181817]">
      ...
      <section className="px-6 pb-20 md:px-12 md:pb-24"><div className="mx-auto max-w-[1392px]">
        <div className="flex flex-wrap items-center justify-between gap-5 border-b border-[#c9c4ba] pb-3"><div className="flex flex-wrap gap-x-12 gap-y-2"><FilterButton>Series (2)</FilterButton><FilterButton>Width</FilterButton><FilterButton>Finish</FilterButton></div><div className="flex items-center gap-7 text-[12px] font-light"><button type="button">All filters (2)</button><span className="h-7 w-px bg-[#c9c4ba]" /><button type="button">Sort by ↓</button></div></div>
        <div className="mt-4 border-b border-[#c9c4ba] pb-5"><p className="text-[9px] font-semibold uppercase tracking-[0.22em]">Selected filters (2)</p><div className="mt-3 flex flex-wrap items-center gap-2 text-[12px]"><span className="rounded-full border border-[#8d877d] px-4 py-2 font-semibold">Single Oven ×</span><span className="rounded-full border border-[#8d877d] px-4 py-2 font-semibold">Double Oven ×</span><button type="button" className="ml-2 underline underline-offset-4">Clear all</button></div></div>
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3">{models.map((oven, index) => <OvenCard key={oven.model} oven={oven} priority={index < 3} />)}</div><p className="mt-10 text-center text-[11px] font-semibold">Viewing 10 of 10</p>
      </div></section>
