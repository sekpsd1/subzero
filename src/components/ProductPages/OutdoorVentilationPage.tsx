...
export function OutdoorVentilationPage() {
  return (
    <main className="bg-[#f4f2ec] text-[#181817]" style={{ fontFamily: '"Segoe UI", Arial, Helvetica, sans-serif' }}>
      <section className="relative h-[520px] overflow-hidden bg-[#0c0c0b] text-white md:h-[100svh] md:max-h-[1080px]">
        <Image src={assets.hero} alt="Premium outdoor kitchen with Wolf outdoor ventilation" fill preload sizes="100vw" className="object-cover object-center" />
        ...
        <div className="mx-auto max-w-[1392px]">
          <div className="flex flex-wrap items-center justify-between gap-5 border-b border-[#c9c4ba] pb-3 text-[13px] font-light">
            <div className="flex flex-wrap gap-x-12 gap-y-2">
              {["Series", "Width"].map((filter) => <button key={filter} type="button" className="inline-flex min-h-9 items-center gap-3">{filter}<span aria-hidden="true">⌄</span></button>)}
            </div>
            <div className="flex items-center gap-8"><button type="button">All filters (3)</button><button type="button">Sort by ↓</button></div>
          </div>

          <div className="pb-[70px] pt-4 text-[11px]">
            <p className="font-semibold uppercase tracking-[0.18em]">Selected filters (1)</p>
            <div className="mt-3 flex items-center gap-4">
              <span className="rounded-full border border-[#8d877d] px-3 py-1.5">Ventilation ×</span>
              <Link href="/cooking/outdoor" className="underline underline-offset-4">Clear all</Link>
            </div>
          </div>

          <section aria-label="Ventilation products">
            <div className="grid gap-1 sm:grid-cols-2 lg:grid-cols-3">
              <article className="w-full max-w-[459px] border border-[#d2cdc1] bg-transparent p-5 md:p-6 md:pb-[14px]">
                <Link href="/products/cooking/outdoor/pwo3318" className="group block focus:outline-none focus-visible:ring-1 focus-visible:ring-[#181817]">
                  <div className="relative aspect-[4/5] overflow-hidden md:h-[598px] md:aspect-auto">
                    <Image src={assets.wallHood} alt="Wolf Outdoor Pro Wall Hood" fill preload sizes="(min-width: 1024px) 408px, (min-width: 640px) 46vw, 88vw" className="object-contain transition duration-500 group-hover:scale-[1.02]" />
                  </div>
                  <div className="flex min-h-9 justify-end py-2 text-[12px] font-light">1 / 3</div>
                  <h2 style={serif} className="pb-4 text-[20px] font-light leading-[1.2] underline-offset-4 group-hover:underline md:text-[24px]">Outdoor Pro Wall Hood</h2>
                </Link>
                <dl className="mt-3 border-t border-[#d8d2c8] text-[12px] font-light leading-[1.35]">
                  <div className="grid grid-cols-2 py-2"><dt>Width:</dt><dd>36&quot;&nbsp; 48&quot;&nbsp; 60&quot;</dd></div>
                </dl>
              </article>
            </div>
      ...
      <section className="px-6 pb-16 pt-4 md:px-12 md:pb-20 md:pt-4">
        <div className="mx-auto grid max-w-[1392px] gap-8 lg:grid-cols-[1.28fr_1fr] lg:gap-20">
        ...
        <div className="mx-auto max-w-[1392px]">
          ...
          <div className="mt-10 grid gap-2">
            <article className="grid bg-[#fbfaf6] md:grid-cols-[0.255fr_0.745fr]">
              <div className="flex min-h-[360px] flex-col items-start justify-between p-6 md:min-h-[500px]"><div><h3 style={serif} className="text-[30px] font-light leading-[1.1] md:text-[40px]">Cleaner air for fresher food</h3><p className="mt-6 max-w-[306px] text-[13px] font-light leading-[1.35] md:text-[14px]">Iconic Sub-Zero Refrigeration preserves food at its delicious peak and guards wine against its enemies.</p></div><PillLink href="/refrigeration/discover-sub-zero">Explore Sub-Zero</PillLink></div>
              <div className="relative min-h-[360px] overflow-hidden md:min-h-[500px]"><Image src={assets.subZero} alt="Sub-Zero refrigeration integrated into an outdoor kitchen" fill sizes="(min-width: 768px) 75vw, 100vw" className="object-cover" /></div>
            </article>
            <div className="grid gap-1 md:grid-cols-2 md:gap-2"><DiscoveryCard title="Effortless control, flawless flavor" copy="Wolf equips cooks of every level with the confidence to create exquisite meals time and time again." cta="Explore Wolf" href="/cooking/discover-wolf" image={assets.wolf} /><DiscoveryCard title="A pristine and peaceful clean" copy="Your silent partner in the kitchen, Cove's intuitive, customizable features offer a worry-free wash and dry." cta="Explore Cove" href="/dishwashing/discover-cove" image={assets.cove} /></div>
          </div>
        </div>
      </section>
    </main>
