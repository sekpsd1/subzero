...
export function OutdoorSideBurnersPage() {
  return (
    <main className="bg-[#f4f2ec] text-[#181817]" style={{ fontFamily: '"Segoe UI", Arial, Helvetica, sans-serif' }}>
      <section className="relative h-[520px] overflow-hidden bg-[#0c0c0b] text-white md:h-[100svh] md:max-h-[1080px]">
        <Image src={assets.hero} alt="Premium outdoor kitchen with Wolf cooking appliances" fill preload sizes="100vw" className="object-cover object-center" />
      ...
      <section className="px-6 pb-16 md:px-12">
        <div className="mx-auto max-w-[1392px]">
          <div className="relative mt-4 flex flex-wrap items-start justify-between gap-5 border-b border-[#c9c4ba] pb-4 text-[13px] font-light">
            <div className="flex flex-wrap gap-x-12 gap-y-2">
              <button type="button" className="inline-flex h-10 items-center gap-3 px-4">Series (2)<span aria-hidden="true">⌄</span></button>
              <button type="button" className="inline-flex h-10 items-center gap-3 px-4">Width<span aria-hidden="true">⌄</span></button>
            </div>
            <div className="flex h-10 items-center gap-5"><button type="button" className="px-3">All filters (2) ⚙</button><span className="h-9 border-l border-[#d2cdc1]" aria-hidden="true" /><button type="button" className="px-3">Sort by ↓</button></div>
          </div>

          <div className="mt-4 pb-6 text-[12px]">
            <p className="pb-4 font-semibold uppercase tracking-[0.18em]">Selected filters (2)</p>
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex min-h-10 items-center rounded-full border border-[#181817] px-3 text-[13px] font-medium">Burner Module <span className="ml-2 text-[20px] font-light leading-none" aria-hidden="true">×</span></span>
              <span className="inline-flex min-h-10 items-center rounded-full border border-[#181817] px-3 text-[13px] font-medium">Side Burner <span className="ml-2 text-[20px] font-light leading-none" aria-hidden="true">×</span></span>
              <Link href="/cooking/outdoor" className="ml-1 px-3 py-2 underline underline-offset-4">Clear all</Link>
            </div>
          </div>

          <div className="grid gap-2 pt-6 sm:grid-cols-2 lg:grid-cols-3">
            {products.map((product, index) => (
              <article key={product.model} className="w-full max-w-[459px] border border-[#d2cdc1] bg-transparent p-5 md:p-6 md:pb-[14px]">
                <Link href={product.href} className="group block focus:outline-none focus-visible:ring-1 focus-visible:ring-[#181817]">
                  <div className="relative aspect-[4/5] overflow-hidden md:h-[598px] md:aspect-auto">
                    <Image src={product.image} alt={`Wolf ${product.title}`} fill preload={index === 0} sizes="(min-width: 1024px) 408px, (min-width: 640px) 46vw, 88vw" className="object-contain transition duration-500 group-hover:scale-[1.02]" />
                  </div>
                  <h3 style={serif} className="max-w-[410px] pb-4 text-[20px] font-light leading-[1.2] underline-offset-4 group-hover:underline md:text-[24px]">{product.title}</h3>
                </Link>
                <dl className="mt-3 border-t border-[#d8d2c8] text-[12px] font-light leading-[1.35]">
                  <div className="grid grid-cols-2 border-b border-[#d8d2c8] py-2"><dt>Width:</dt><dd>13&quot;</dd></div>
                  <div className="grid grid-cols-2 py-2"><dt>Fuel Type:</dt><dd>Liquid Propane<br />Natural Gas</dd></div>
                </dl>
              </article>
            ))}
          </div>
          <p className="mt-[68px] text-center text-[11px] font-semibold">Viewing 2 of 2</p>
        </div>
      </section>

      <section className="px-6 pb-16 pt-4 md:px-12 md:pb-14">
        <div className="mx-auto grid max-w-[1392px] gap-8 lg:grid-cols-[1.28fr_1fr] lg:gap-20">
          ...
          <p style={serif} className="max-w-[500px] text-[18px] font-light leading-[1.2] md:text-[24px]">With Sub-Zero and Wolf, you can picture yourself preparing the same gourmet meals outdoors as you do indoors. Our multi-function grill gives you masterful control, while warming drawers lend a helping hand. Sub-Zero&apos;s convenient undercounter refrigeration keeps fresh foods, cold drinks, ice, and other necessities at your fingertips.</p>
        </div>
      </section>

      <section className="bg-[#d3d0c5] px-6 pb-16 pt-16 md:px-12 md:pt-20">
        <div className="mx-auto max-w-[1392px]">
